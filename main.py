"""
نظام التداول الورقي المتكامل - مع لوحة تحكم ويب
============================================================================
الميزات الجديدة:
- خادم ويب مدمج على المنفذ 10000
- صفحة HTML تعرض الصفقات المفتوحة والمغلقة وإشارات Gold
- تحديث تلقائي كل 30 ثانية
- متوافق مع Render Web Service
"""
...
# =========================================================
# المؤشرات الفنية (نفسها بدون تغيير)
# =========================================================
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        # تعطيل سجلات HTTP لتجنب الإزعاج
        pass

def generate_dashboard_html():
    """توليد صفحة HTML للوحة التحكم"""
    stats = generate_api_stats()
    ...
    <head>
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            h2 {{ margin-bottom: 15px; color: #00d4ff; border-bottom: 2px solid #00d4ff; padding-bottom: 5px; }}
            .stats-grid {{
                display: grid;
            ...
            .badge-pending {{ background: #ffcc00; color: #1a1a2e; }}
        </style>
        <script>
            function refreshData() {{
                fetch('/api/stats')
                    ...
                    .then(data => {{
                        ...
                        document.getElementById('pnl').textContent = (data.total_pnl >= 0 ? '+' : '') + '$' + data.total_pnl.toFixed(2);
                        document.getElementById('pnl').className = 'stat-value ' + (data.total_pnl >= 0 ? 'positive' : 'negative');
                        document.getElementById('return_pct').textContent = (data.total_return_pct >= 0 ? '+' : '') + data.total_return_pct.toFixed(2) + '%';
                        document.getElementById('return_pct').className = 'stat-value ' + (data.total_return_pct >= 0 ? 'positive' : 'negative');
                        ...
                        document.getElementById('gold_signals_count').textContent = data.gold_signals.length;
    ...
    <body>
        <div class="container">
            <h1>🤖 لوحة تحكم بوت التداول الورقي</h1>
            
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-value" id="cash">${stats['cash']:.2f}</div>
                    <div class="stat-label">الرصيد الحالي</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" id="pnl" class="{('positive' if stats['total_pnl'] >= 0 else 'negative')}">{('+' if stats['total_pnl'] >= 0 else '')}${stats['total_pnl']:.2f}</div>
                    <div class="stat-label">صافي الربح</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" id="return_pct" class="{('positive' if stats['total_return_pct'] >= 0 else 'negative')}">{('+' if stats['total_return_pct'] >= 0 else '')}{stats['total_return_pct']:.2f}%</div>
                    <div class="stat-label">العائد</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" id="total_trades">{stats['total_trades']}</div>
                    <div class="stat-label">إجمالي الصفقات</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" id="win_rate">{stats['win_rate']:.1f}%</div>
                    <div class="stat-label">نسبة النجاح</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" id="open_positions">{stats['open_positions']}</div>
                    <div class="stat-label">صفقات مفتوحة</div>
                </div>
            </div>
            
            <h2>🥇 إشارات Gold الحالية</h2>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>#</th><th>الرمز</th><th>السعر</th><th>هدف %</th><th>ألفا</th><th>ثقة</th><th>أولوية</th>
                        </tr>
                    </thead>
                    <tbody>
    """
    
    for i, sig in enumerate(stats['gold_signals'][:10], 1):
        confidence_class = 'confidence-high' if sig['confidence'] >= 70 else ('confidence-medium' if sig['confidence'] >= 50 else 'confidence-low')
        html += f"""
                        <tr>
                            <td>{i}</td>
                            <td><strong>{sig['symbol']}</strong></td>
                            <td>${sig['entry_price']:.4f}</td>
                            <td>{sig['target_pct']:.2f}%</td>
                            <td>{sig['alpha']:.3f}</td>
                            <td class="{confidence_class}">{sig['confidence']}%</td>
                            <td>{sig['priority']}</td>
                        </tr>
        """
    
    if not stats['gold_signals']:
        html += """
                        <tr><td colspan="7" style="text-align: center;">لا توجد إشارات حالياً</td></tr>
        """
    ...
    html += f"""
            ...
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>الرمز</th><th>سعر الدخول</th><th>السعر الحالي</th><th>الكمية</th><th>الربح/الخسارة</th><th>نسبة %</th><th>نوع الإشارة</th><th>ثقة</th>
                        </tr>
                    </thead>
                    <tbody>
    """
    ...
    for pos in stats['open_positions']:
        ...
        html += f"""
                        <tr>
                            ...
                            <td>{pos['amount']:.4f}</td>
                            <td class="{pnl_class}">{('+' if pos['pnl'] >= 0 else '')}${pos['pnl']:.2f}</td>
                            <td class="{pnl_class}">{('+' if pos['pnl_pct'] >= 0 else '')}{pos['pnl_pct']:.2f}%</td>
    ...
    if not stats['open_positions']:
        html += """
                        <tr><td colspan="8" style="text-align: center;">لا توجد صفقات مفتوحة</td></tr>
        """
    ...
    html += f"""
            ...
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>الرمز</th><th>سعر الدخول</th><th>سعر الخروج</th><th>الربح/الخسارة</th><th>نسبة %</th><th>سبب الإغلاق</th><th>التوقيت</th>
                        </tr>
                    </thead>
                    <tbody>
    """
    
    for trade in stats['closed_trades'][-10:]:
        ...
        html += f"""
                        <tr>
                            ...
                            <td class="{pnl_class}">{('+' if trade['pnl'] >= 0 else '')}${trade['pnl']:.2f}</td>
                            <td class="{pnl_class}">{('+' if trade['pnl_pct'] >= 0 else '')}{trade['pnl_pct']:.2f}%</td>
                            <td>{trade['exit_reason']}</td>
                            <td>{trade['exit_time'][:16]}</td>
                        </tr>
        """
    
    if not stats['closed_trades']:
        html += """
                        <tr><td colspan="7" style="text-align: center;">لا توجد صفقات مغلقة</td></tr>
        """
    ...
    html += f"""
            ...
            <div class="refresh-info">
                🟢 البوت يعمل | آخر تحديث: <span id="last_update">{datetime.now().strftime('%H:%M:%S')}</span> | التحديث تلقائي كل 30 ثانية
            </div>
        </div>
    </body>
    </html>
...
def generate_api_stats():
    """توليد إحصائيات API للوحة التحكم"""
    global trader_instance, candidate_tracker, last_gold_signals
    ...
    if trader_instance:
        trader_stats = trader_instance.get_stats()
        if trader_stats:
            stats['cash'] = trader_instance.cash
            stats['total_pnl'] = trader_stats['total_pnl']
            stats['total_return_pct'] = trader_stats['total_return_pct']
            stats['total_trades'] = trader_stats['total_trades']
            stats['win_rate'] = trader_stats['win_rate']
            stats['open_positions'] = trader_stats['open_positions']
        ...
        for pos in trader_instance.positions:
            ...
            stats['open_positions_list'].append({
                'symbol': pos['symbol'],
                'entry_price': pos['entry_price'],
        ...
        stats['open_positions'] = stats['open_positions_list']
        
        # الصفقات المغلقة
        for trade in trader_instance.closed_trades[-20:]:
    ...
    # إشارات Gold
    if candidate_tracker:
        for cand in candidate_tracker.active_candidates:
            if cand.get('tier') == 'Gold' and cand['status'] == 'ACTIVE':
                stats['gold_signals'].append({
                    'symbol': cand['symbol'],
                    'entry_price': cand['entry_price'],
        ...
        stats['gold_signals'].sort(key=lambda x: x['priority'], reverse=True)
...
def run_web_server():
    ...
    print(f"🌐 خادم الويب يعمل على المنفذ {web_server_port}")
...
async def process_candidates(candidates, trader, exchange_sync, tracker, tier_name):
    ...
    # حفظ إشارات Gold للوحة التحكم
    if tier_name == "Gold":
        last_gold_signals = candidates[:10]
...
async def trading_loop():
    ...
    print(f"🤖 بدء التداول مع لوحة تحكم ويب - {datetime.now()}\n")
    await asyncio.sleep(2)
    await send_telegram_message(f"🚀 بوت التداول مع لوحة تحكم ويب بدأ العمل!\n🌐 لوحة التحكم: http://localhost:{web_server_port}\n🥇 Gold: {len(gold_symbols)} عملة (كل 5 دقائق)")
